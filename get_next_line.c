...
/*   Updated: 2022/11/12 16:38:47 by mrichard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_read(int fd, char *str)
{
	...
	array = malloc(sizeof(char) * (BUFFER_SIZE + 1)); //maloco o tamanho necessario para o buffer 
	if (!array) //caso o malloc falhe
		return (NULL);
	...
	while (!ft_strchr(array, '\n') && bytes != 0) //enquanto nao achar o \n e ainda tiver algo pra ler
	{
		...
		array[bytes] = '\0'; //finalizo o buffer com null byte
		str = ft_strjoin(str, array); //faco a juncao do que acabei de ler com o q ja tinha no str
	}
...
char	*get_next_line(int fd)
{
	...
	free(line);
	s = ft_new_line(s); //guardo o que sobrou desse reads
